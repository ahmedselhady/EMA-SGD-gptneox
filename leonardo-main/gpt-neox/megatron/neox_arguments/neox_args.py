...
class NeoXArgsModel(NeoXArgsTemplate):
    ...
    # """
    # Not used. Added for model conversion.
    # """
...
class NeoXArgsTraining(NeoXArgsTemplate):
    ...
    global_num_epochs: int = None
    """
    The number of epochs for which to train on the entire dataset. The model will be trained on all datasets for the same number of epochs. 

    WARNING: only works when train_data_paths have been set!
    WARNING: setting this to a value will override train_iters!!!

    Overrides weight_by_num_documents, and will use weight_by_num_tokens in any case.
    """

    weight_by_num_tokens: bool = False
    """
    If True, Builds dataset weights from a multinomial distribution over groups of data according to the number of tokens in each group. Overrides weight_by_num_documents.
